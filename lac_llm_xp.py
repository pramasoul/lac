...
def provide_model(model_name="internal", device="cpu", threads=1):
    return load_to_xp(model_name + ".npz") # FIXME: device
...
class LLMPredictionService(PredictionService):
    ...
    def __init__(self, llm_predictor, idx=xp.array([[198]])):
    ...
    def accept(self, tok):
        ...
        if self.idx.size < self.p.model_config.block_size:
            ...
            self.idx = xp.concatenate((self.idx, xp.array([[tok]])), axis=-1)
        else:
            if self._temp_idx is None:
                #self._temp_idx = self.idx.clone()
                self._temp_idx = xp.asarray(self.idx, copy=True)
            t = self._temp_idx
            ...
            xp.copyto(self.idx, t)   # FIXME: Should ping-pong between them instead?
...
class LLMPredictor:
    ...
    def __call__(self, idx):
        logits = forward_model_dict(self.model_dict, xp.asarray(idx))
