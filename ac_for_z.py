...
class ACSampler:
    def __init__(self, end_of_text_token=None, precision=48):
        self.eot_token = end_of_text_token
        ...
        self.compress_output = (
            None  # Set to a function to be called with each bit as compressed
        )
        self.decompress_bits = iter(())  # Set to a provider of the bits to decompress
        self.decompress_output = (
            None  # Optionally a function to be called with each decompressed token
        )
        self.bits_per_token = (
            None  # Optional function to observe the instantaneous bits per token
        )
        self.on_decompress_done = (
            None  # Optional callback after all bits decompressed and tokens returned
        )
            ...
            minpdf := np.min(
                mpdf := np.diff(
                    np.concatenate(
                        (
                            np.array([0]),
                            self.region.map(cdf.astype(float), float(cdf[-1]))
                            - self.region.low,
                        )
                    )
                )
...
def compress_base_ten(digits):
    ...
    def bpt(v, s=[0, 0]):
        ...
        s[1] += v
        print(
            f"\x1b[Kcompressed {s[0]} digits to {s[1]} bits, bpt={v}, avgbpt = {s[1]/s[0]}",
            end="\r",
        )
