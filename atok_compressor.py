...
class ACTokCompressor:
    ...
    def compress(self, data, *args, **kwargs):
        ...
        if not self.header_sent:
            logging.debug(f"ACTokCompressor.compress including header in rv")
            rv = self._header()
        ...
        rv += v
        logging.debug(f"ACTokCompressor.compress {self=} {len(rv)=} {rv[:16]}[...]{rv[-16:]} {eot_ix=}")
        return rv
    ...
    def flush(self, *args):
        ...
        rv += self._footer()
        logging.debug(f"{rv=}")
        logging.debug(f"ACTokCompressor.flush {self=} {len(rv)=} {rv[:16]}[...]{rv[-16:]}")
        return rv
